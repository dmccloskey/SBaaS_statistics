...
class data_stage02_quantification_svm(Base):
    ...
    def __repr__json__(self):
        return json.dumps(self.__repr__dict__())
class data_stage02_quantification_svm_features(Base):
    __tablename__ = 'data_stage02_quantification_svm_features'
    id = Column(Integer, Sequence('data_stage02_quantification_svm_features_id_seq'), primary_key=True)
    analysis_id = Column(String(500))
    component_group_name = Column(String(100))
    component_name = Column(String(500))
    impfeat_value = Column(Float);
