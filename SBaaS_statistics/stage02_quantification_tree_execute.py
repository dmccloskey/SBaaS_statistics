...
class stage02_quantification_tree_execute(stage02_quantification_tree_io):
        ...
        for cu in calculated_concentration_units:
            ...
            for row in response_class_methods_I:
                if row['response_class_method'] == 'class_probability':
                    ...
                    dictList = {};
                    for i,response in enumerate(response_label_tmp):
                        dictList[response]=calculateinterface.data_train['row_labels']['sample_name_short']
                    data_O_listDict.set_dictList(dictList);
                    data_O_listDict.convert_dictList2DataFrame();
    ...
    def execute_treeCrossValidation(self,analysis_id_I,
                tree_model_I='RandomForestClassifier',
                tree_method_I="scikit-learn",
                tree_parameters_I=None,
                metric_method_I = None,
                metric_options_I = None,
                metric_score_I = None,
                crossval_method_I = None,
                crossval_options_I = None,
                crossval_parameters_I = None,
                calculated_concentration_units_I=[],
                experiment_ids_I=[],
                sample_name_abbreviations_I=[],
                sample_name_shorts_I=[],
        ...
        INPUT:
        analysis_id
        tree_model_I = string, 'DecisionTreeClassifier', 'RandomForestClassifier', 'ExtraTreesClassifier', or 'AdaBoostClassifier'
        tree_method_I = string, 'scikit-learn'
        tree_parameters_I = {}, default=None, which will use recommended settings
        impfeat_method_I = 'feature_importance',
        impfeat_options_I = {}, default=None,
        OPTIONAL INPUT:
        ...
        ...
        for cu in calculated_concentration_units:
            ...
            data_O_listDict = listDict();
            # call the tree method
            calculateinterface.make_dataModel(tree_model_I,tree_parameters_I);
            tree_parameters_I = calculateinterface.data_model.get_params(); #update the parameters
            # extract important features
            if impfeat_method_I == 'feature_importance':
                impfeat_value = calculateinterface.extract_importantFeatures();
                impfeat_n,impfeat_std = calculateinterface.calculate_importantFeatures_std(impfeat_value);
                impfeat_zscore,impfeat_pvalue = calculateinterface.calculate_ZScoreAndPValue(
                    impfeat_value,impfeat_n,impfeat_std);
                ...
                dataFeatureSelection = calculateinterface.make_dataFeatureSelection(
                        tree_model_I,tree_parameters_I,
            ...
            # add in additional rows to the output data object
            data_O_listDict.add_column2DataFrame('component_name',calculateinterface.data['column_labels']['component_name'].ravel());
            data_O_listDict.add_column2DataFrame('component_name',calculateinterface.data['column_labels']['component_group_name'].ravel());
            data_O_listDict.add_column2DataFrame('analysis_id',analysis_id_I);
            data_O_listDict.add_column2DataFrame('used_',True);
            data_O_listDict.add_column2DataFrame('comment_',None);
            data_O_listDict.add_column2DataFrame('tree_model',tree_model_I);
            data_O_listDict.add_column2DataFrame('tree_method',tree_method_I);
            data_O_listDict.add_column2DataFrame('tree_parameters',tree_parameters_I);
            data_O_listDict.add_column2DataFrame('impfeat_method',impfeat_method_I);
            data_O_listDict.add_column2DataFrame('tree_method',impfeat_options_I);
            # add data to the database
            data_O_listDict.convert_dataFrame2ListDict();
            self.add_rows_table('data_stage02_quantification_tree_features',data_O_listDict.get_listDict());
            #data_features_O.append(impfeat_listDict.get_listDict());
            # reset calculate_interface
            calculateinterface.clear_data();
